...
async def start(c, m, cb=False):
    ...
    owner_username = owner.username else 'Plutonium X'
    ...
    if len(m.command) > 1: # sending the stored file
        ...
        if 'batch_' in m.command[1]:
            ...
            if string.empty:
                owner = await c.get_users(int(OWNER_ID))
                return await m.reply_text(f"**The file has been deleted by the uploader!**")
        ...
        if msg.empty:
            return await send_msg.edit(f"**The file has been deleted by the uploader!**")
