...
async def forcesub(c, m):
    ...
    if UPDATE_CHANNEL:
        try:
            ...
            buttons = [[InlineKeyboardButton(text='🔑 sᴜʙsᴄʀɪʙᴇ ᴅᴜɴɪᴀᴍᴏᴠɪᴇ', url=f"https://t.me/{UPDATE_CHANNEL}")]]
            if m.text:
                if (len(m.text.split(' ')) > 1) & ('start' in m.text):
                    chat_id, msg_id = m.text.split(' ')[1].split('_')
                    buttons.append([InlineKeyboardButton('⚡️ ʀᴇғʀᴇsʜ', callback_data=f'refresh+{chat_id}+{msg_id}')])
            await m.reply_text(
            ...
            print(e)
            await m.reply_text(f"sepertinya ada yang salah 🧐, pastian anda menambahkan saya sebagai admin.. {owner.mention(style='md')}", quote=True)
            return
...
async def refresh_cb(c, m):
    ...
    if UPDATE_CHANNEL:
        try:
            ...
            print(e)
            await m.message.edit(f"sepertinya ada yang salah 🧐, pastian anda menambahkan saya sebagai admin.. {owner.mention(style='md')}")
            return
